// 単に S の元の列を管理する
template <typename S, bool PERSISTENT=false>
struct RBST {
  struct Node {
    Node *l, *r;
    S s;
  ...
  // 区間 [l, r) を取り出し, [0, l), [l, r), [r, size)に分ける
  ...
  np set(np root, uint k, const S &s) { return set_rec(root, k, s); }
  S get(np root, uint k) { return get_rec(root, k, false); }
...
private:
  ...
  np set_rec(np root, uint k, const S &s) {
    if (!root) return root;
    prop(root);
    uint sl = (root->l ? root->l->size : 0);
    if (k < sl) {
  ...
  S get_rec(np root, uint k, bool rev) {
