...
struct SplayTree {
  ...
  uint get_size(np root) { return (root ? root->size : 0); }
  ...
  void goto_between(np &root, uint l, uint r) {
  ...
  X get(np &root, uint k) {
    assert(root == nullptr || !root->p);
    splay_kth(root, k);
    return root->get();
  }

  void set(np &root, uint k, const X &x) {
    assert(root != nullptr && !root->p);
    splay_kth(root, k);
    root->set(x);
  }

  void multiply(np &root, uint k, const X &x) {
    assert(root != nullptr && !root->p);
    splay_kth(root, k);
    root->multiply(x);
  }
  ...
  void apply(np &root, uint l, uint r, const A &a) {
    if (l == r) return;
    assert(0 <= l && l < r && r <= root->size);
    goto_between(root, l, r);
    root->apply(a);
  ...
  void reverse(np &root, uint l, uint r) {
    assert(root == nullptr || !root->p);
    if (l == r) return;
    assert(0 <= l && l < r && r <= root->size);
    goto_between(root, l, r);
