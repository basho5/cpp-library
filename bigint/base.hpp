...
struct BigInteger {
  ...
  BigInteger(string s) {
    ...
    int m = ceil(n, LOG);
    dat.assign(m, 0);
    rep(i, n) { dat[i / LOG] += TEN[i % LOG] * (s[i] - '0'); }
  }
  ...
  bool operator<(const bint &p) const {
    ...
    if (sz(dat) != sz(p.dat)) {
      if (sgn == 1) return sz(dat) < sz(p.dat);
      if (sgn == -1) return sz(dat) > sz(p.dat);
    }
    drep(i, sz(dat)) {
  ...
  bint &operator+=(const bint p) {
    ...
    int n = max(sz(dat), sz(p.dat));
    dat.resize(n + 1);
    rep(i, n) {
      if (i < sz(p.dat)) dat[i] += p.dat[i];
      if (dat[i] >= MOD) dat[i] -= MOD, dat[i + 1] += 1;
    }
    while (sz(dat) && dat.back() == 0) dat.pop_back();
    if (dat.empty()) sgn = 0;
    return *this;
  }
  bint &operator-=(const bint p) {
  ...
  vc<int> convolve(const vc<int> &a, const vc<int> &b) {
    ...
    vc<mint0> a0(rng(a)), b0(rng(b));
    vc<mint1> a1(rng(a)), b1(rng(b));
    vc<mint2> a2(rng(a)), b2(rng(b));
  ...
  string to_binary_string() {
    ...
    string ANS;
    while (1) {
      while (sz(A) && A.back() == uint(0)) A.pop_back() ;
      if (A.empty()) break;
      ull rem = 0;
      drep(i, sz(A)) {
        rem = rem * MOD + A[i];
  ...
  pair<bint, int> divmod(int p) {
    ...
    reverse(rng(after));
    while (sz(after) && after.back() == 0) after.pop_back() ;
    bint q;
    q.sgn = sgn;
    q.dat = after;
    rm *= sgn;
  ...
  double log10() {
    ...
    double x = 0;
    rep(i, 4) x = MOD * x + dat[sz(dat) - 1 - i];
    x = std::log10(x);
    x += double(LOG) * (sz(dat) - 4);
    return x;
  }
