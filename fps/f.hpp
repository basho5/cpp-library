...
vc<mint> convolution(const vc<mint>& a, const vc<mint>& b) {
  int n = sz(a), m = sz(b);
  if (!n || !m) return {};
	// NTTで扱えるサイズは 2^(mint::ntt_info().fi) まで
  const int max_ntt_size = 1 << mint::ntt_info().fi;
  if (n > max_ntt_size || m > max_ntt_size) {
    // サイズが大きすぎるので、NTTを使わず Garner 法に切り替える
...
#define s int(vm::size())
 ...
 //   // f/g. f の長さで出力される.
  ...
  fps pow1(mint k, int deg) const {return pow_1(d, k, deg);}
  fps pow1(mint k) const {return pow_1(d, k, s);}
